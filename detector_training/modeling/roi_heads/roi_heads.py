# modified from https://github.com/facebookresearch/CutLER/blob/main/cutler/modeling/roi_heads/roi_heads.py
...
class ROIHeads(torch.nn.Module):
    ...
    def label_and_sample_proposals(
        ...
        storage = get_event_storage()
        storage.put_scalar("roi_head/num_fg_samples_{}".format(branch), np.mean(num_fg_samples))
        storage.put_scalar("roi_head/num_bg_samples_{}".format(branch), np.mean(num_bg_samples))
...
class CustomStandardROIHeads(ROIHeads):
    ...
    def forward(
        ...
        if (self.training and compute_loss) or compute_val_loss:
            losses, _ = self._forward_box(features, proposals,
                branch=branch, compute_loss=compute_loss, compute_val_loss=compute_val_loss)
            # Usually the original proposals used by the box head are used by the mask, keypoint
            # heads. But when `self.train_on_pred_boxes is True`, proposals will contain boxes
            # predicted by the box head.
            losses.update(self._forward_mask(features, proposals, compute_loss, compute_val_loss))
            losses.update(self._forward_keypoint(features, proposals, compute_loss, compute_val_loss))
    ...
    def forward_with_given_boxes(
        self, features: Dict[str, torch.Tensor], instances: List[Instances],
        compute_loss: bool = True, compute_val_loss: bool = False,
    ) -> List[Instances]:
        """
        Use the given boxes in `instances` to produce other (non-box) per-ROI outputs.
        ...
        instances = self._forward_mask(features, instances, compute_loss, compute_val_loss)
        instances = self._forward_keypoint(features, instances, compute_loss, compute_val_loss)
        return instances
    ...
    def _forward_box(
        ...
        no_gt_found = False
        if self.use_droploss and ((self.training and compute_loss) or compute_val_loss):
        ...
        if (self.training and compute_loss) or compute_val_loss:
            if self.use_droploss and not no_gt_found:
                ...
                losses = self.box_predictor.losses(predictions, proposals, branch=branch, weights=weights.detach())
            else:
                losses = self.box_predictor.losses(predictions, proposals, branch=branch)
    ...
    def _forward_mask(self, features: Dict[str, torch.Tensor], instances: List[Instances],
        ...
        """
        if not self.mask_on:
            return {} if ((self.training and compute_loss) or compute_val_loss) else instances
        ...
        if self.mask_pooler is not None:
            ...
            boxes = [x.proposal_boxes if ((self.training and compute_loss) or compute_val_loss) else x.pred_boxes for x in instances]
        ...
        if (self.training and compute_loss) or compute_val_loss:
            ...
            # Hack: switch mask head to eval mode
            training_mode = self.mask_head.training
            self.mask_head.eval()
            instances = self.mask_head(features, instances)
            self.mask_head.train(training_mode)
            return instances
    ...
    def _forward_keypoint(self, features: Dict[str, torch.Tensor], instances: List[Instances],
        ...
        if (self.training and compute_loss) or compute_val_loss:
            ...
            # Hack: switch keypoint head to eval mode
            training_mode = self.keypoint_head.training
            self.keypoint_head.eval()
            instances = self.keypoint_head(features, instances)
            self.keypoint_head.train(training_mode)
            return instances
    ...
    def label_and_sample_proposals_pseudo(
        ...
        for proposals_per_image, targets_per_image, confids_per_image in zip(proposals, targets, gt_confids):
            ...
            if has_gt:
                ...
                proposals_per_image.set("gt_confid", torch.ones_like(sampled_idxs))
