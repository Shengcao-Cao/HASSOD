# modified from https://github.com/facebookresearch/CutLER/blob/main/cutler/engine/defaults.py
...
class DefaultPredictor:
    ...
    def __init__(self, cfg):
        ...
        self.model.eval()
        if cfg.SEMISUPNET.TRAINER == "mean_teacher":
            self.model_teacher = build_model(self.cfg)
            self.model_teacher.eval()
        if len(cfg.DATASETS.TEST):
            self.metadata = MetadataCatalog.get(cfg.DATASETS.TEST[0])

        if cfg.SEMISUPNET.TRAINER == "mean_teacher":
            ensem_ts_model = EnsembleTSModel(self.model_teacher, self.model)
            checkpointer = DetectionTSCheckpointer(ensem_ts_model)
        else:
    ...
    def __call__(self, original_image):
        ...
        with torch.no_grad():  # https://github.com/sphinx-doc/sphinx/issues/4258
            ...
            if self.cfg.SEMISUPNET.TRAINER == "mean_teacher":
                predictions = self.model_teacher([inputs])[0]
            else:
